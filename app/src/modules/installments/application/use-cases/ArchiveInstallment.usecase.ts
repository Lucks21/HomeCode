...
import { InstallmentNotFoundException, InstallmentNotFullyPaidException } from '../../domain/exceptions';
...
@Injectable()
export class ArchiveInstallmentUseCase {
  constructor(
  ...
  async execute(id: number, userId: number): Promise<Installment> {
    ...
    if (!installment.isFullyPaid) {
      throw new InstallmentNotFullyPaidException();
    }
